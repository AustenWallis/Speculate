...
from Speculate_addons.Spec_functions import plot_emulator
from Speculate_addons.Spec_functions import simplex

# %% 2 Setting up and processing the flux grid space (HDF5). 
...
emu_file_name = 'SSpec_emu_60inc_1510-1590A_4comp_123'  # Emulator name format: 
...
if kgrid == 1:
    ...
    usecols = (1, 8) # wavelength and inclination column. Angle changeable with column 8-14 --> 40 to 70 degrees
...
if shortspec == 1:
    ...
    usecols = (1, 16) # wavelength and inclination column. Angle changeable with column 10-21 --> 30 to 85 degrees
    skiprows = 81 # Start of data within file
...
plot_emulator(emu, grid, model_parameters, 1, 0) # Displayed parameter (1-X), other parameters fixed index (0-(X-1))
...
for i in tqdm(range(len(emu.grid_points))): # Iterating across the spectral file grid points (changing file names) (y-axis)
    ...
    waves, obs_flux_data = np.loadtxt(spectrum_file, usecols=usecols, unpack = True, skiprows=skiprows)
...
# 4)
if data_four == 1:
    file = 'runtest_WMdot2e-10_d14_vinf1p5.spec'              # Python v87a formatting. Place python file within kgrid folder/directory

### ------------------------------------|

if data_one == 1: 
    waves, fluxes = np.loadtxt(f'kgrid/sscyg_kgrid090311.210901/{file}', usecols=usecols, unpack = True, skiprows=skiprows)
...
if data_four == 1:
    waves, fluxes = np.loadtxt(f'observation_files/{file}', unpack = True, usecols=usecols, skiprows=skiprows)
...
### ----- Inputs here ------|
log_amp = -55               # Natural logarithm of the global covariance's Matern 3/2 kernel amplitude log=-52 'linear', log=-8 'log'
log_ls = 6                 # 5Natural logarithm of the global covariance's Matern 3/2 kernel lengthscale
### ------------------------|

model = SpectrumModel(
    f'Grid-Emulator_Files/{emu_file_name}.hdf5',
    data,
    grid_params=list(emu.grid_points[50]), #[list, of , grid , points]emu.grid_points[119] [-8.95, 10.26, 1.82]
    Av=0,
    global_cov=dict(log_amp=log_amp, log_ls=log_ls)
)
print(model)
...
if shortspec == 1:
        default_priors = {
        "param1": st.uniform(np.log10(4e-11),(np.log10(3e-9)-np.log10(4e-11))), # log10 values
...
initial_simplex = simplex(model, priors) # need to add global covariance hyperparameters
model.train(priors, options=dict(maxiter=1e5, disp=True, initial_simplex=initial_simplex, return_all=True))
print(model)
...
max_n = 400                # Maximum iterations of the MCMC if convergence is not reached.
